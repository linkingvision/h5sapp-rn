...
import { View, Text, ActivityIndicator, Alert, TextInput, StyleSheet,Dimensions } from 'react-native'
...
const Ceshi = () => {
    ...
    function selectEvent(checked,data) {
        if (checked) {
            if (data.strName=="根节点") {
                ...
                if (!checkedEvent) {
                    ...
                    for (var key in treelist) {
                        ...
                        treelist[key].node.map((item)=>{
                            ...
                            seleceEventdata.push(item.strToken)
                ...
                seleceEventdata.push(data.strToken)
                store.changeEventToken(seleceEventdata);
    ...
    return (
        <View style={{backgroundColor: store.data.backgroundColor,height:1700}}>
        ...
        borderRadius:0.048*WIDTH,}}>
                ...
                <TextInput style={{paddingLeft:0.048*WIDTH,
        fontSize:0.043*WIDTH,
        height: 0.12*WIDTH,color:store.data.TextColor,}} placeholderTextColor={store.data.TextColor}  placeholder="请输入关键字进行检索" value={search} onChangeText={(value) => { setSearch(value); console.log(search) }}></TextInput>
